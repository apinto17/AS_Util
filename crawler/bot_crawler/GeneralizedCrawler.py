...
def init(site, start, end):
    ...
    logging.basicConfig(
        ...
        filename=site.name + "/" + site.name + ".log",
        level=logging.DEBUG,
    )
    logging.info(
        "Thread: " + str(site.thread) + " start: " + str(start) + " end: " + str(end)
    )
...
def test(site, link, func, arg):
    ...
    if type(arg) is not str:
        ...
        if not site.is_cat_page():
            raise ValueError(
                "Second argument must be the link for the page of categories"
            )
        cats = site.get_cats()
        for cat in cats:
            print("----------------------------------------")
        ...
        if not site.is_prod_page():
            raise ValueError(
                "Second argument must be the link for the page of products"
            )
        ...
        raise ValueError(
            'Fourth argument not recognized, must be either "browser", "cat", or "item"'
        )
