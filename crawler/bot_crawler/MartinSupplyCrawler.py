...
class MartinSupplyCrawler(st.Site):
    ...
    def get_item_price(self, item):
        ...
        try:
            price = self.browser.find_element_by_css_selector(
                "div.cart-cost"
            ).text.strip()
        except:
            pass
    ...
    def get_item_unit(self, item):
        ...
        self.follow_url(self.get_item_link(item))
        try:
            unit_whole = self.browser.find_elements_by_css_selector("div.cart-option")[
                -1
            ]
    ...
    def get_item_specs(self, item=None):
        ...
        for spec in specs:
            key = (
                spec.find_element_by_css_selector("strong")
                .text.strip()
                .replace(":", "")
            )
