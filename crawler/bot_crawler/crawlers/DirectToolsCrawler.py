...
class DirectToolsCrawler(st.Site):
    ...
    def get_show_all_page(self):
        return self.browser.find_element_by_css_selector("nav.column > ul > li:nth-last-child(1) > a")
    ...
    def get_item_image(self, item):
        img = None
        try:
            img = item.find_element_by_css_selector("span.imgh > img").get_attribute("src")
        except:
            pass 
        return img
    ...
    def get_item_specs(self, item=None):
        res = {}
        try:
            specs_header = self.browser.find_element_by_xpath("//*[contains(text(), 'Specifications')]")
